...
export class PokemonService { //Creo la clase del servidor que en este caso es Pokemon Service
  ...
  findPokemon(search) { //Funcion para buscar pokemon
    return this.http.get(`${this.baseUrl}/pokemon/${search}`).pipe(//Le digo que me retorne los datos de pokemon al hacer una busqueda con search
      map(pokemon => { //Mapear para obtener datos especificos
        pokemon['image'] = this.getPokeImage(pokemon['id']);//Obtener la imagen por id (Busqueda por id)
        pokemon['pokeIndex'] = pokemon['id'];//Obetner los datos de id (Busqueda por id)
        return pokemon;
      })
    );
  }
